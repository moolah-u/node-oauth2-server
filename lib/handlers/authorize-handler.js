...
AuthorizeHandler.prototype.generateAuthorizationCode = function() {
  if (this.model.generateAuthorizationCode) {
    return Promise.try(promisify(this.model.generateAuthorizationCode));
  }
...
AuthorizeHandler.prototype.getClient = function(request) {
  ...
  return Promise.try(promisify(this.model.getClient, 2), clientId)
...
AuthorizeHandler.prototype.getUser = function(request, response) {
  ...
  return Promise.try(promisify(this.authenticateHandler.handle, 2), [request, response]).then(function(user) {
    if (!user) {
...
AuthorizeHandler.prototype.saveAuthorizationCode = function(authorizationCode, expiresAt, scope, client, redirectUri, user) {
  ...
  return Promise.try(promisify(this.model.saveAuthorizationCode, 3), [code, client, user]);
};
